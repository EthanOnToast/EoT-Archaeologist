...
program
    ...
    try {
        ...
        for (const commit of commits.slice(0, 10)) {
            console.log(`${chalk.gray(commit.hash.substring(0, 8))} ` +
                `${commit.date.substring(0, 10)} ` +
                `${commit.author}`);
            console.log(`  ${commit.message}`);
            console.log();
        }
        console.log(chalk.gray(`Showing ${Math.min(commits.length, 10)} commits.`));
    }
    catch (error) {
        console.error();
        console.error(chalk.red("Could not inspect this file."));
        console.error(chalk.gray("Make sure you're running the command inside a Git repository."));
    }
});
program.parseAsync();
